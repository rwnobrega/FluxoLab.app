...
import { Position, XYPosition, Dimensions } from 'reactflow'
...
export default function getPath (
  ...
  switch (pos0) {
    case Position.Bottom: {
      switch (pos1) {
        case Position.Top: {
          if (bottom0 < top1) { // Cases 6, 7, 8
            const yp = (y0 + y1) / 2
            path = [[x0, y0], [x0, yp], [x1, yp], [x1, y1]]
            break
          }
          let xp: number
          if (left0 > right1) { // Cases 1, 4
            xp = (left0 + right1) / 2
          } else if (right0 < left1) { // Cases 3, 5
            xp = (right0 + left1) / 2
          } else if (x0 >= x1) { // Case 2a
            xp = Math.min(left0, left1) - gridSize
          } else { // Case 2b
            xp = Math.max(right0, right1) + gridSize
          }
          path = [[x0, y0], [x0, y0 + gridSize], [xp, y0 + gridSize], [xp, y1 - gridSize], [x1, y1 - gridSize], [x1, y1]]
          break
        }
        case Position.Left: {
          if (bottom0 <= top1 && x0 < left1) { // Case 7b, 8
            path = [[x0, y0], [x0, y1], [x1, y1]]
            break
          }
          let xp, yp: number
          if (right0 < left1) { // Cases 3, 5
            xp = (right0 + left1) / 2
            yp = y0 + gridSize
          } else if (top0 > bottom1) { // Cases 1, 2
            xp = Math.min(left0, left1) - gridSize
            yp = Math.max(bottom0, bottom1) + gridSize
          } else if (bottom0 < top1) { // Cases 6, 7a
            xp = left1 - gridSize
            yp = (bottom0 + top1) / 2
          } else { // Case 4
            xp = Math.min(left0, left1) - gridSize
          ...
          path = [[x0, y0], [x0, yp], [xp, yp], [xp, y1], [x1, y1]]
          break
        }
        case Position.Right: { // Use symmetry
          const newCenter0 = { x: -center0.x, y: center0.y }
          const newCenter1 = { x: -center1.x, y: center1.y }
          return _.map(
            getPath(Position.Bottom, Position.Left, newCenter0, newCenter1, dim0, dim1),
            ([x, y]) => [-x, y]
          )
        }
      }
    ...
    case Position.Right: {
      switch (pos1) {
        case Position.Top: { // Use symmetry
          const newCenter0 = { x: center0.x, y: -center0.y }
          const newCenter1 = { x: center1.x, y: -center1.y }
          return _.reverse(
            _.map(
              getPath(Position.Bottom, Position.Right, newCenter1, newCenter0, dim1, dim0),
              ([x, y]) => [x, -y]
            )
          )
        }
        case Position.Left: {
          if (right0 < left1) { // Cases 3, 5, 8
            const xp = (x0 + x1) / 2
            path = [[x0, y0], [xp, y0], [xp, y1], [x1, y1]]
            break
          }
          let xp, yp: number
          if (top0 > bottom1 || bottom0 < top1) { // Cases 1, 2, 6, 7
            xp = x1 - gridSize
            yp = (y0 + y1) / 2
          } else if (y0 > center1.y) { // Case 4a
            xp = left1 - gridSize
            yp = top1 - gridSize
          } else { // Case 4b
            xp = left1 - gridSize
          ...
          path = [[x0, y0], [x0 + gridSize, y0], [x0 + gridSize, yp], [xp, yp], [xp, y1], [x1, y1]]
          break
        }
        case Position.Right: {
          if (bottom0 <= top1 || top0 >= bottom1) { // Cases 1, 2, 3, 6, 7, 8
            const xp = Math.max(right0, right1) + gridSize
            path = [[x0, y0], [xp, y0], [xp, y1], [x1, y1]]
          } else if (right0 < left1) { // Case 5
            const xp = (right0 + left1) / 2
            const yp = bottom1 + gridSize
            path = [[x0, y0], [xp, y0], [xp, yp], [x1 + gridSize, yp], [x1 + gridSize, y1], [x1, y1]]
          } else { // Case 4
...
function removeRedundantPoints (path: Array<[number, number]>): Array<[number, number]> {
